...
public class PropertiesBuilder {
  /**
   * Creates a text containing all the relevant properties of a JSmoothModelBean object. The properties are output in the form "key=value".
   * <p>
   * 
   * Note that all the paths are converted to be made relative to the basedir parameter provided. All the paths converted are expected to be
   * relative to the targetted executable binary (before the conversion is applied, that is).
   * 
   */
  static public String makeProperties(File basedir, JSmoothModelBean obj) {
    ...
    // if (obj.getBundledJVMPath() != null) addPair("bundledvm", getRenormalizedPathIfNeeded(obj.getBundledJVMPath(), basedir, curdir),
    // out);
  ...
  /**
   * Converts a path relative to previousbasedir into a path relative to newbasedir.
   */
  static public String getRenormalizedPathIfNeeded(String value, File previousbasedir, File newbasedir) {
    ...
    if (newbasedir == null) return value;
